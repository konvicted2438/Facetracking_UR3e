...
cap.set(700, frameWidth)
cap.set(400, frameHeight)
cap.set(100,150)

face_detect_model = cv2.dnn.readNetFromCaffe("MODELS/deploy.prototxt.txt", "MODELS/res10_300x300_ssd_iter_140000.caffemodel")
...
ACCELERATION = 0.05  # Robot acceleration value
...
def set_lookorigin():
    ...
    position1 = (position[0],position[1],position[2],position[3],position[4],position[5])
    print(position1)
    orig = m3d.Transform(position1)
    return orig
...
def move_to_face(list_of_facepos,robot_pos):
    ...
    #print("oriented_xyz_coord",oriented_xyz_coord)
    coordinates = oriented_xyz_coord

    qnear = robot.get_actual_joint_positions()
    next_pose = coordinates
    #print(next_pose)
    #pose_10 = coordinates.get_pose_vector()
    #pose_11 = coordinates[1]
    #pose_12 = coordinates[2]
    #pose_13 = coordinates[3]
    #pose_14 = coordinates[4]
    #print(pose_10)
    #print("poses:",pose_10,pose_11,pose_12,pose_13,pose_14)
...
try:
    ...
    while True:
        ...
        face_positions, new_frame = find_faces_dnn(img)
